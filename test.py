...
def find_pure_regions(point_class_distribution: Dict[tuple, Dict[str, int]]) -> set:
    """
    Identifies pure regions where only a single class exists in the space.
    
    Args:
        point_class_distribution (Dict[tuple, Dict[str, int]]): Class distribution at each lattice point.
    
    Returns:
        set: A set of lattice points that are pure (single class).
    """
    pure_regions = set()
    for point, class_distribution in point_class_distribution.items():
        if len(class_distribution) == 1:
            pure_regions.add(point)
    return pure_regions
...
def plot_3d_spheres(data: pd.DataFrame, class_column: str = 'class', grid_size=100) -> None:
    ...
    pure_regions = find_pure_regions(point_class_distribution)
        ...
        updatemenus=[
            {
                "buttons": [
                    {
                        "args": [None, {"opacity": 0.1}],
                        "label": "Highlight Pure Regions",
                        "method": "update"
                    },
                    {
                        "args": [None, {"opacity": 0.8}],
                        "label": "Reset Opacity",
                        "method": "update"
                    }
                ],
